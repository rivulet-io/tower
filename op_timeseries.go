...
func (tw *Tower) TimeSeriesCreate(key string) error {
	unlock := tw.lock(key)
	defer unlock()
...
func (tw *Tower) TimeSeriesDelete(key string) error {
	...
	return tw.db.Delete([]byte(key), &pebble.WriteOptions{Sync: false})
}
...
func (tw *Tower) TimeSeriesAdd(key string, timestamp time.Time, value PrimitiveData) error {
	unlock := tw.lock(key)
	defer unlock()
	...
	err = tw.db.Set(dataPointKey, valueBytes, &pebble.WriteOptions{Sync: false})
	if err != nil {
...
func (tw *Tower) TimeSeriesGet(key string, timestamp time.Time) (PrimitiveData, error) {
	...
	// Get the data point
	value, closer, err := tw.db.Get(dataPointKey)
	if err != nil {
...
func (tw *Tower) TimeSeriesRemove(key string, timestamp time.Time) error {
	unlock := tw.lock(key)
	defer unlock()
	...
	_, closer, err := tw.db.Get(dataPointKey)
	if err != nil {
		if err == pebble.ErrNotFound {
	...
	err = tw.db.Delete(dataPointKey, &pebble.WriteOptions{Sync: false})
	if err != nil {
...
func (tw *Tower) TimeSeriesRange(key string, startTime, endTime time.Time) (map[time.Time]PrimitiveData, error) {
	...
	iter, err := tw.db.NewIter(&pebble.IterOptions{
