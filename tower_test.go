...
func TestNewTower(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			options: &Options{
				Path:         "test_data",
				FS:           OnDisk(),
				CacheSize:    NewSizeFromMegabytes(32),
				MemTableSize: NewSizeFromMegabytes(8),
				BytesPerSync: NewSizeFromKilobytes(256),
			},
			wantErr: false,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			tower, err := NewTower(tt.options)
			if (err != nil) != tt.wantErr {
				t.Errorf("NewTower() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if tower != nil {
				defer tower.Close()
