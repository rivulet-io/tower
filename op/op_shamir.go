...
func (op *Operator) SetShamirShare(key string, shares map[byte][]byte) error {
	unlock := op.lock(key)
	defer unlock()
...
func (op *Operator) CombineSharesFrom(shares map[byte][]byte) ([]byte, error) {
	if len(shares) == 0 {
...
func (op *Operator) AddShare(key string, shareID byte, share []byte) error {
	unlock := op.lock(key)
	defer unlock()
