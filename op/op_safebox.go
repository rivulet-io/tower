...
func (op *Operator) UpsertSafeBox(key string, data []byte, encKey []byte, algorithm EncryptionAlgorithm) ([]byte, error) {
	unlock := op.lock(key)
	defer unlock()
...
func (op *Operator) ExtractSafeBox(key string, encKey []byte) ([]byte, error) {
	algorithm, encryptedData, nonce, err := op.GetSafeBox(key)
	if err != nil {
		return nil, err
	}
